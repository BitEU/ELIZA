...
:BUILD_START
echo.
echo Building ELIZA/DOCTOR Simulation...
echo.
...
if exist eliza_univac.exe del /Q eliza_univac.exe
if exist ELIZA_univac.o del /Q ELIZA_univac.o
if exist *.o del /Q *.o
echo.
...
echo Compiling ELIZA.c...
gcc -c -DUNIVAC -O2 -Wall ELIZA.c -o ELIZA_univac.o

if %ERRORLEVEL% NEQ 0 (
    echo ERROR: Failed to compile ELIZA.c
    pause
    exit /b 1
)

echo Linking...
gcc -o eliza_univac.exe ELIZA_univac.o
...
REM Clean previous build artifacts
echo Cleaning previous build artifacts...
if exist eliza_mingw.exe del /Q eliza_mingw.exe
if exist ELIZA.obj del /Q ELIZA.obj
if exist *.o del /Q *.o
echo.
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: eliza_mingw.exe

    REM Display file size
    for %%A in (eliza_mingw.exe) do (
        echo File size: %%~zA bytes
    )
    echo.
    echo Optimization level: MAXIMUM (-O3 + LTO + native CPU^)
    echo.
    echo To run ELIZA, type: eliza_mingw.exe
    echo.
) else (
    echo.
    echo ========================================
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: eliza.exe
    echo.
    echo To run: eliza.exe
) else (
    echo.
    echo ========================================
    echo   BUILD FAILED
